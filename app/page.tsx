...
export default function HomePage() {
  ...
  const [regionDetails, setRegionDetails] = useState<
    { name: string; stateCount: number; centerCount: number }[]
  >([]);
    ...
    async function fetchStates() {
      try {
        ...
        if (
          ...
          Object.keys(regionStateMapping).length === 0
        ) {
          console.log(
            "No regions found, attempting to reinitialize data mappings..."
          );
          const success = await reinitializeDataMappings();

          if (success) {
            // Load the data again after reinitialization
            const [updatedRegionsWithDetails, updatedRegionStateMapping] =
              await Promise.all([
                getRegionsWithDetails(),
                getRegionToStateMapping(),
              ]);
            ...
            console.log(
              `Data reinitialized. Found ${
                Object.keys(updatedRegionStateMapping).length
              } regions`
            );
          }
        }
