...
export function HomeCoursesCarousel({
  ...
  return (
    <div
      className={`jb-course-carousel${peek === 'roomy' ? ' jb-course-carousel--roomy' : ''}`}
    >
      <div
        ref={scrollerRef}
        className="jb-course-carousel__track"
        role="list"
      ...
      >
        {items.map((item) => {
          const body = (
            <>
              <div className="jb-course-carousel__square">
                {item.src ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={item.src} alt={item.alt} loading="lazy" decoding="async" />
                ) : (
                  <div className="jb-course-carousel__ph" aria-hidden />
                )}
              </div>
              <div className="jb-course-carousel__meta">
                <h3 className="jb-course-carousel__title">{item.title}</h3>
                {item.blurb ? <p className="jb-course-carousel__blurb">{item.blurb}</p> : null}
              </div>
            </>
          );
          ...
          if (item.href) {
            return (
              <Link
                key={item.id}
                href={item.href}
                className="jb-course-carousel__card jb-course-carousel__card--link"
                data-course-card
                role="listitem"
              >
                {body}
          ...
          return (
            <article
              key={item.id}
              className="jb-course-carousel__card"
              data-course-card
              role="listitem"
            >
              {body}
      ...
      <div className="jb-course-carousel__footer">
        <div className="jb-course-carousel__dots" role="tablist" aria-label={`${ariaLabel} slides`}>
          {items.map((item, i) => (
            <button
              key={item.id}
              type="button"
              ...
              className={`jb-course-carousel__dot${i === active ? ' is-active' : ''}`}
              onClick={() => {
                const el = scrollerRef.current;
        ...
        <div className="jb-course-carousel__nav">
          <button
            type="button"
