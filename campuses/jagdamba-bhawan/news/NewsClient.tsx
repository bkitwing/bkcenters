...
import type { JbNewsCategory, JbNewsPageData, JbNewsPost } from '../jb-media-data';
import { JbMediaHero } from '../JbMediaHero';
import { NewsMapAnimation } from './NewsMapAnimation';

const ARCHIVE_ID = 'jb-news-archive';
const ALL_ID = '__all__';

function NewsCard({ post }: { post: JbNewsPost }) {
  return (
    <article className={`jb-news-card${post.Featured ? ' jb-news-card--featured' : ''}`}>
      <a
        href={post.href}
        target="_blank"
        rel="noopener noreferrer"
        className="jb-news-card__link"
        aria-label={`Read: ${post.title}`}
      >
        <div className="jb-news-card__media">
          {post.imageUrl ? (
            ...
            <img
              ...
              alt={post.imageAlt}
              loading="lazy"
              className="jb-news-card__img"
            />
          ) : (
            <div className="jb-news-card__placeholder" aria-hidden />
          )}
          {post.hasVideo ? (
            <span className="jb-news-card__badge" aria-label="Video">
              <Play className="h-3 w-3" />
            </span>
          ) : null}
          {post.Featured ? (
            <span className="jb-news-card__badge jb-news-card__badge--feat">Featured</span>
          ) : null}
        </div>
        <div className="jb-news-card__body">
          <p className="jb-news-card__meta">
            <time dateTime={post.date}>{post.dateLabel}</time>
          </p>
          <h3 className="jb-news-card__title">{post.title}</h3>
          <span className="jb-news-card__out">
            Read on news site
...
function NewsCategoryNav({
  ...
  return (
    <nav className="jb-gal-nav jb-news-nav" aria-label="News categories">
      <div className="jb-gal-nav__track" ref={trackRef} role="group">
        <button
          type="button"
          data-chip-id={ALL_ID}
          className={`jb-gal-pill${activeId === ALL_ID ? ' is-active' : ''}`}
        ...
        {categories.map((cat) => (
          <button
            ...
            type="button"
            data-chip-id={cat.id}
            className={`jb-gal-pill${activeId === cat.id ? ' is-active' : ''}`}
...
export default function NewsClient({ data }: { data: JbNewsPageData }) {
    ...
    return (
      <div className="jb-news">
        <JbMediaHero
          ...
          title="Service News"
          lede="Campus news and seva stories from Jagdamba Bhawan Retreat Center will appear here as they are published."
          animation={<NewsMapAnimation />}
        />
        <div className="jb-container pb-16">
          <div className="jb-media-empty">
            <p className="jb-media-empty__title">News stories coming soon</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="jb-news">
      <JbMediaHero
        ...
        title="Service News"
        lede="Retreats, conferences, seva and campus highlights from Jagdamba Bhawan Retreat Center — stories from the Brahma Kumaris news desk."
        animation={<NewsMapAnimation />}
        actions={
          <a href={`#${ARCHIVE_ID}`} className="jb-media-hero__btn jb-media-hero__btn--ghost">
            Browse archive
          </a>
        }
      />

      <div className="jb-container jb-news__body pb-16 md:pb-24">
        {data.latest.length > 0 ? (
          <section className="jb-news-latest" aria-labelledby="jb-news-latest-title">
            <h2 id="jb-news-latest-title" className="jb-news-latest__title">
              Latest stories
            </h2>
            <div className="jb-news-grid jb-news-grid--4">
              {data.latest.map((p) => (
        ...
        <div id={ARCHIVE_ID} className="jb-news-archive">
          {categories.length > 0 ? (
            categories.map((cat, i) => (
              <section
                ...
                className={`jb-news-category${activeCategory === cat.id ? ' is-active' : ''}`}
                aria-labelledby={`${cat.id}-title`}
              >
                <header className="jb-news-category__head">
                  <span className="jb-news-category__index" aria-hidden>
                    {String(i + 1).padStart(2, '0')}
                  </span>
                  <h3 id={`${cat.id}-title`} className="jb-news-category__title">
                    {cat.label}
                  </h3>
                  <span className="jb-news-category__count">{cat.posts.length}</span>
                </header>
