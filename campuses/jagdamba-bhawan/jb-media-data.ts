/**
 * Jagdamba Bhawan news + events — portal Strapi (public list endpoints).
 *
 * News tag 79 = Pune - Jagdamba Bhawan
 * Event organizer 26 = Jagdamba Bhawan Retreat Center - Pune
 *
...
export const JB_NEWS_TAG_ID = 79;
export const JB_EVENT_ORGANIZER_ID = 26;
...
export type JbNewsTag = {
  id: number;
  name: string;
  href: string;
};

export type JbNewsPost = NewsPost & {
  dateLabel: string;
  imageUrl: string;
...
export type JbNewsCategory = {
  id: string;
  label: string;
  posts: JbNewsPost[];
};

export type JbEventPost = EventPost & {
  dateLabel: string;
  imageUrl: string;
  imageAlt: string;
  href: string;
...
export type JbEventsPageData = {
  organizerName: string;
  organizerSlug: string;
  description: string;
  total: number;
  events: JbEventPost[];
  upcoming: JbEventPost[];
  ongoing: JbEventPost[];
  past: JbEventPost[];
  heroImage: string | null;
  page: number;
...
function eventStatus(start: string, end: string): JbEventPost['status'] {
  const now = Date.now();
...
function mapNewsTags(raw: unknown): JbNewsTag[] {
  if (!Array.isArray(raw)) return [];
  const tags: JbNewsTag[] = [];
...
const JB_NEWS_GENERAL_CATEGORY = {
  id: 'campus-stories',
  label: 'Campus stories',
} as const;

function buildNewsCategories(posts: JbNewsPost[]): JbNewsCategory[] {
  const map = new Map<string, JbNewsCategory>();

  for (const post of posts) {
    const topicTags = post.tags.filter((t) => t.id !== JB_NEWS_TAG_ID);
    if (topicTags.length === 0) {
      const bucket = map.get(JB_NEWS_GENERAL_CATEGORY.id) ?? {
        id: JB_NEWS_GENERAL_CATEGORY.id,
        label: JB_NEWS_GENERAL_CATEGORY.label,
        posts: [],
      };
...
function mapNewsPost(raw: Record<string, unknown>): JbNewsPost | null {
...
function mapEventPost(raw: Record<string, unknown>): JbEventPost | null {
  const id = typeof raw.id === 'number' ? raw.id : Number(raw.id);
  const title = typeof raw.title === 'string' ? raw.title : '';
  const slug = typeof raw.slug === 'string' ? raw.slug : '';
...
export const getJbNews = cache(async (): Promise<JbNewsPageData> => {
  const empty: JbNewsPageData = {
    tagName: 'Pune - Jagdamba Bhawan',
    tagHref: 'pune-jagadamba-bhawan',
    total: 0,
    posts: [],
    latest: [],
    categories: [],
  ...
  const [tagRes, postsRes] = await Promise.all([
    portalGet<{ data: unknown }>(`/news-tags/${JB_NEWS_TAG_ID}`),
    portalGet<{ data: unknown[]; meta?: { pagination?: { total?: number } } }>(
      `/news-posts?` +
        [
          `filters[post_tags][id][$eq]=${JB_NEWS_TAG_ID}`,
  ...
  const posts = (postsRes?.data || [])
    ...
    .filter((p): p is JbNewsPost => Boolean(p));
...
function bucketJbEvents(events: JbEventPost[]) {
...
function eventsListPath(page: number, pageSize: number) {
  return (
    `/events?` +
    [
      `filters[event_organizors][id][$eq]=${JB_EVENT_ORGANIZER_ID}`,
...
export async function fetchJbEventsPage(
  ...
  const events = (eventsRes?.data || [])
    ...
    .filter((e): e is JbEventPost => Boolean(e));
...
export const getJbEvents = cache(async (): Promise<JbEventsPageData> => {
  const empty: JbEventsPageData = {
    organizerName: 'Jagdamba Bhawan Retreat Center - Pune',
    organizerSlug: 'jagdamba-bhawan-retreat-center-pune',
    description: '',
    total: 0,
    events: [],
    upcoming: [],
  ...
  const { upcoming, ongoing, past } = bucketJbEvents(eventsBatch.events);
