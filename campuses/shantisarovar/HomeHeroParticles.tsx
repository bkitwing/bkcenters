...
/** Soft golden dust motes — visible but light (canvas, pauses off-screen). */
...
export function HomeHeroParticles() {
  ...
  useEffect(() => {
    ...
    const drawMote = (p: Particle, alpha: number) => {
      if (p.soft) {
        ...
        g.addColorStop(0, `rgba(255, 232, 160, ${alpha})`);
        g.addColorStop(0.45, `rgba(255, 200, 100, ${alpha * 0.55})`);
        g.addColorStop(1, `rgba(184, 149, 90, 0)`);
        ctx.fillStyle = g;
        ctx.beginPath();
        ...
        ctx.fillStyle = `rgba(255, 215, 122, ${alpha})`;
        ctx.fill();
      }
    };
  ...
  return <canvas ref={canvasRef} className="ss-hero-particles" aria-hidden />;
}
