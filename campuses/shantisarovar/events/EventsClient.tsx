...
import type { SsEventPost, SsEventsPageData } from '../ss-media-data';
import { useLoadMore } from '../hooks/useLoadMore';
import { useSsEventsFetchMore } from '../hooks/useSsEventsFetchMore';
import { SsMediaHero } from '../SsMediaHero';
import { EventsAuraAnimation } from './EventsAuraAnimation';

const LIST_ID = 'ss-events-list';
...
function buildGroups(events: SsEventPost[]): EventGroup[] {
  const map: Record<EventGroupId, SsEventPost[]> = {
    upcoming: events
...
function mergeEvents(existing: SsEventPost[], incoming: SsEventPost[]): SsEventPost[] {
...
function EventCard({ event }: { event: SsEventPost }) {
  const canRegister =
    (event.status === 'upcoming' || event.status === 'ongoing') &&
    isHttpUrl(event.registration_link);

  return (
    <article className={`ss-event-card ss-event-card--${event.status}`}>
      <div className="ss-event-card__media">
        {event.imageUrl ? (
          ...
          <img
            ...
            alt={event.imageAlt}
            loading="lazy"
            className="ss-event-card__img"
          />
        ) : (
          <div className="ss-event-card__placeholder" aria-hidden />
        )}
        <span className={`ss-event-card__status ss-event-card__status--${event.status}`}>
          {event.status}
        </span>
      </div>
      <div className="ss-event-card__body">
        <p className="ss-event-card__meta">
          <Calendar className="w-3.5 h-3.5 shrink-0" aria-hidden />
          <time dateTime={event.start_date}>{event.dateLabel}</time>
        </p>
        <h3 className="ss-event-card__title">{event.title}</h3>
        <div className="ss-event-card__actions">
          <a
            href={event.href}
            target="_blank"
            rel="noopener noreferrer"
            className="ss-event-card__btn ss-event-card__btn--ghost"
          >
            Know more
          ...
          {canRegister ? (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="ss-event-card__btn ss-event-card__btn--primary"
            >
              Register
            </a>
          ) : null}
...
function EventGroupSection({
  ...
  return (
    <section
      id={group.id}
      ref={sectionRef}
      className={`ss-events-group${activeId === group.id ? ' is-active' : ''}`}
      aria-label={`${group.label} events`}
    >
      <div className="ss-events-grid">
...
function EventsScheduleNav({
  ...
  return (
    <nav className="ss-gal-nav ss-gal-nav--compact ss-events-nav" aria-label="Event schedule">
      <div className="ss-gal-nav__track" ref={trackRef} role="group">
        {groups.map((g) => (
          <button
            ...
            type="button"
            data-chip-id={g.id}
            className={`ss-gal-pill${activeId === g.id ? ' is-active' : ''}${
...
export default function EventsClient({ data }: { data: SsEventsPageData }) {
  ...
  const onApiBatch = useCallback((batch: { events: SsEventPost[]; page: number; hasMore: boolean }) => {
    setAllEvents((prev) => mergeEvents(prev, batch.events));
    setPage(batch.page);
    setHasMore(batch.hasMore);
  }, []);

  const { sentinelRef: apiSentinelRef } = useSsEventsFetchMore({
    page,
  ...
  return (
    <div className="ss-events">
      <SsMediaHero
        ...
        title="Campus Events"
        lede="Rajyoga retreats, workshops and campus programmes at Shanti Sarovar — what's happening now and what's ahead."
        animation={<EventsAuraAnimation />}
        actions={
          allEvents.length > 0 ? (
            <a href={`#${landingId}`} className="ss-media-hero__btn ss-media-hero__btn--ghost">
              View schedule
            </a>
          ) : undefined
        }
      ...
      <div className="ss-container ss-events__body pb-16 md:pb-24">
        {allEvents.length === 0 ? (
          <div className="ss-media-empty">
            <p className="ss-media-empty__title">Events coming soon</p>
            <p className="ss-media-empty__desc">
          ...
          <>
            ...
            <div id={LIST_ID} className="ss-events-list">
                ...
                <EventGroupSection
                  ...
                  apiSentinel={
                    hasMore ? (
                      <div
                        ref={apiSentinelRef}
                        className="ss-load-sentinel"
                        aria-hidden="true"
                      >
                        <div className="ss-load-sentinel__dots">
                          <span />
                          <span />
                          <span />
                        </div>
