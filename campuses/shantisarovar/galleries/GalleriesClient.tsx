...
function GalleryLightbox({
  ...
  return (
    ...
    >
      <div className="ss-gal-lb__shell">
        <header className="ss-gal-lb__bar">
          <p className="ss-gal-lb__count" aria-live="polite">
            {index + 1} / {total}
          </p>
          <div className="ss-gal-lb__tools">
            <button
              type="button"
              className="ss-gal-lb__icon"
              onClick={zoomOut}
            ...
            >
              <span className="ss-gal-lb__zoom-label">{Math.round(zoom * 100)}%</span>
            </button>
            <button
              type="button"
              className="ss-gal-lb__icon"
            ...
            <button
              ref={closeRef}
              type="button"
              className="ss-gal-lb__icon ss-gal-lb__icon--close"
              onClick={onClose}
        ...
        <div className="ss-gal-lb__viewport">
          <div
            className={`ss-gal-lb__stage${zoom > 1 ? ' is-zoomed' : ''}`}
            onWheel={onWheel}
          ...
          >
            {image.src ? (
              ...
              <img
                ...
                src={image.src}
                alt={image.alt}
                className={`ss-gal-lb__img${loaded ? ' is-loaded' : ''}`}
                width={image.width}
                height={image.height}
                decoding="async"
                draggable={false}
          ...
          {canPrev ? (
            <button
              type="button"
              className="ss-gal-lb__nav ss-gal-lb__nav--prev"
              onClick={(e) => {
                e.stopPropagation();
                onChange(index - 1);
              }}
          ...
          {canNext ? (
            <button
              type="button"
              className="ss-gal-lb__nav ss-gal-lb__nav--next"
              onClick={(e) => {
                e.stopPropagation();
                onChange(index + 1);
              }}
        ...
        {total > 1 ? (
          <footer className="ss-gal-lb__thumbs">
            <div
              ref={thumbStripRef}
              className="ss-gal-lb__thumb-track"
              role="listbox"
            ...
            >
              {images.map((img, i) => (
                <button
                  ...
                  className={`ss-gal-lb__thumb${i === index ? ' is-active' : ''}`}
                  onClick={() => onChange(i)}
                >
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
...
function GalleryStickyNav({
  ...
  return (
    <nav className="ss-gal-nav" aria-label={ariaLabel}>
      <div className="ss-gal-nav__track" ref={onTrackEl} role="group">
        {showAll ? (
          <button
            type="button"
            data-chip-id={ALL_ID}
            className={`ss-gal-pill${activeCategory === ALL_ID ? ' is-active' : ''}`}
        ...
        {chips.map((chip) => (
          <button
            ...
            className={`ss-gal-pill${activeCategory === chip.id ? ' is-active' : ''}`}
...
function GalleryCard({
  ...
  return (
    <li
      className={`ss-gal-card ${aspectClass(item.width, item.height, index)}`}
      style={{ '--gal-i': index } as CSSProperties}
    >
      <button type="button" className="ss-gal-card__btn" onClick={onOpen} aria-label={item.alt}>
        <span className="ss-gal-card__skeleton" aria-hidden />
        {src ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img
            src={src}
            ...
            decoding="async"
            className="ss-gal-card__img"
          />
        ) : (
          <span className="ss-gal-card__ph" aria-hidden>
            <TreePine className="w-5 h-5 mb-2 opacity-70" />
            {item.alt}
          </span>
        )}
        <span className="ss-gal-card__hint" aria-hidden>
...
function AlbumBlock({
  ...
  album: SsGalleryAlbum;
  index: number;
  active: boolean;
  forceLoad: boolean;
  onOpen: (items: SsGalleryImage[], i: number) => void;
  ...
  return (
    <section
      id={album.id}
      ref={setRefs}
      className={`ss-gal-section${active ? ' is-active' : ''}`}
      aria-labelledby={`${album.id}-title`}
    >
      <header className="ss-gal-section__head">
        <div className="ss-gal-section__banner">
          <span className="ss-gal-section__index" aria-hidden>
            {String(index + 1).padStart(2, '0')}
          </span>
          <h2 id={`${album.id}-title`} className="ss-gal-section__title">
            {album.title}
          </h2>
        </div>
      </header>
      ...
      {loaded ? (
        ...
        <div
          className="ss-gal-section__placeholder"
          aria-hidden
...
function pickThumbs(albums: SsGalleryAlbum[], limit = 3): string[] {
  const urls: string[] = [];
...
function GroupBlock({
  ...
  return (
    <div
      className={`ss-gal-group${
        groupIndex > 0 ? ' ss-gal-group--service' : ' ss-gal-group--moments'
      }`}
    >
      <GalleryStickyNav
...
export default function GalleriesClient({ data }: { data: SsGalleriesPageData }) {
  const groups = data?.groups ?? [];
  const allAlbums = useMemo(() => groups.flatMap((g) => g.albums), [groups]);
  const albumById = useMemo(() => {
    const map = new Map<string, { album: SsGalleryAlbum; groupIndex: number }>();
    groups.forEach((g, gi) => {
   ...
   * Example: /shantisarovar/galleries#campus
   */
  ...
  const openImage = useCallback((items: SsGalleryImage[], index: number) => {
  ...
  const scrollToGroup = useCallback(
    (groupIndex: number) => {
      ...
      const heroId = `ss-gal-group-${group.id}`;
  ...
  const heroLede =
    primary?.subheading ||
    'Photo collections from Shanti Sarovar — campus, retreats and service.';
    ...
    return (
      <div className="ss-gal">
        <SsMediaHero
          ...
          lede="Photo collections from Shanti Sarovar will appear here soon."
          animation={<GalleriesAuraAnimation />}
        />
        <div className="ss-container pb-16">
          <div className="ss-media-empty mt-10">
            <p className="ss-media-empty__title">Galleries coming soon</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="ss-gal">
            ...
            {jumpLinks.map((link) => (
              <a
                ...
                href={
                  link.groupIndex === 0
                    ? `#${GRID_ID}`
                    : `#ss-gal-group-${link.id}`
                }
                className="ss-media-hero__btn ss-media-hero__btn--ghost"
                onClick={(e) => {
                  e.preventDefault();
      ...
      <div id={GRID_ID} className="ss-gal-flow pb-16 md:pb-24">
        {groups.map((group, gi) => {
          ...
          return (
            <div key={group.id} className="ss-gal-flow__group">
              {gi > 0 ? (
                <SsMediaHero
                  embedded
                  id={`ss-gal-group-${group.id}`}
                  eyebrow={
                    <>
                  ...
                  lede={
                    group.subheading ||
                    'Service, culture and community moments from Shanti Sarovar.'
                  }
              ...
              <div className="ss-container ss-gal-sections">
                <GroupBlock
                  group={group}
                  groupIndex={gi}
